...
async def borrar_estadistica(current_user: Annotated[User, Depends(get_current_active_user)]):
    # 1. Borramos todo el historial de pedidos
    eliminar_ganancias = await db["pedidos"].delete_many({})
    
    # 2. 🌟 ¡Reseteamos el fantasma de las estadísticas a cero!
    await db["stats"].update_one(
        {"tipo": "ingresos_globales"},
        {"$set": {"total_acumulado": 0, "ultima_actualizacion": datetime.utcnow()}},
        upsert=True
    )
